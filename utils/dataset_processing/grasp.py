...
def visualise_output(i, sal, net, title=None):
	...
	q_out, cos_out, sin_out, width_out, sal_out = net(i.unsqueeze(0))
	
	if i.shape[0] == 4:
		image = i.detach().cpu().numpy()[1:,:,:].transpose((1,2,0))
	elif i.shape[0] == 3:
		image = i.detach().cpu().numpy().transpose((1,2,0)) 
	else:
	...
	f_axes[0][0].axis('off')
	f_axes[0][1].imshow(sal.detach().cpu().squeeze().numpy())
	f_axes[0][1].set_title('saliency ground truth')
	f_axes[0][1].axis('off')
	...
	f_axes[1][1].imshow(saliency)
	f_axes[1][1].set_title('saliency output')
	f_axes[1][1].axis('off')
