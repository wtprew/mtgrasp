...
class MTGCNNB3(nn.Module):
	def __init__(self, input_channels=1, num_classes=76, filter_sizes=None, l3_k_size=5, dilations=None):
		...
		self.features3 = nn.Sequential(
			nn.Conv2d(filter_sizes[1], filter_sizes[2], kernel_size=l3_k_size, stride=1, bias=True),
 			nn.ReLU(inplace=True),
			nn.Conv2d(filter_sizes[2], filter_sizes[2], kernel_size=l3_k_size, stride=1, bias=True),
			nn.ReLU(inplace=True),
		)
